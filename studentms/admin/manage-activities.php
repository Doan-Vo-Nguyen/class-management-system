                                        ...
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="font-weight-bold">Activities Name</th>
                                            ...
                                            // Formula for pagination
                                                ...
                                                $sql="SELECT * FROM training_score_activities as tsa JOIN activities as act ON tsa.activity_id = act.activity_id LIMIT $offset, $no_of_records_per_page";
                                                $query = $dbh -> prepare($sql);
                                                $query->execute();
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php
                                                        $sql_count = "SELECT cl.class_name, COUNT(st.student_id) as total_students FROM class as cl LEFT JOIN student as st ON cl.class_id = st.class_id GROUP BY cl.class_id, cl.class_name";
                                                        $query_count = $dbh->prepare($sql_count);
                                                        $query_count->execute();
                                                        $results_count = $query_count->fetchAll(PDO::FETCH_OBJ);
                                                        foreach ($results_count as $row_count) {
                                                            if ($row_count->class_name == $row->class_name) {
                                                            echo htmlentities($row_count->total_students);
                                                            }
                                                        }
                                                        ?>
                                                        <form method="post">
                                                        ...
                                                        <?php 
                                                            if(isset($_POST['submit']))
                                                            {
                                                                ...
                                                                $query = $dbh->prepare($sql);
                                                                $query->execute();
                                                                echo "<script>alert('Student has been confirmed');</script>";
